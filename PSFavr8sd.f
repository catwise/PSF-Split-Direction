c PSFavr8sd: average PSFs for cryo/postcryo and rotate
c            Split-Direction version
c
...
c
     ...
     +              nW2acryo, nW2apsotcryo, nW1dcryo, nW1dpostcryo,
     +              nW2dcryo, nW2dpsotcryo
     ...
     +              OutPath, w1paNam, w2paNam
      ...
      Character*2   TmpStr2
      Character*1   Slash
      Logical       dbg, GotIN, GotOut, w3, w4, da, ge,
     +              GotAng1, GotAng2
c                                                            ! Focal-Plane PSFs
...
c       W2 postcryo will be output as W2 with no epoch averaging; the 
c       W4 (W2 cryo) PSF can be ignored if using option 1b.
c                                                            ! Output awaic PSFs
      Real*4        w1psfawaic(27,27),   w2psfawaic(27,27)   ! W1 & W2
c                                                            ! Internal awaic PSFs
      Real*4        w1psficore(641,641), w2psficore(641,641) ! W1 & W2
c
     ...
     +              MJD1, MJD2
      real*4 nullval, dH, dH1ac, dH1dc, Dh2ac, dH2dc,
     +                    dH1ap, dH1dp, Dh2ap, dH2dp
      logical anynull
c
      common /vdt/ cdate,ctime,vsn
c
      Data Vsn/'1.0  B70804'/, nOvrSamp/11/, nHmax/50/, dH/0.1/
...
c
      NArgs = IArgC()
      If (NArgs .lt. 1) then
        print *,'PSFavr8sd vsn ', Vsn
        print *
        print *,'Usage: PSFavr8sd <flags specifications>'
        print *
...
c                                      ! Dump angle histograms
        ...
        da = .true.
        print *,'Angle histograms wil be dumped to sdtout'
c                                      ! Generate teil epoch text file
...
c                                      ! Histogram resolution
      else if (Flag .eq. '-H') then
        call NextNarg(NArg,Nargs)
        call GetArg(NArg,TmpStr)
...
c
     ...
     +          Ang1apMin,w1psfpa,w1psfuncpa,wgt)
      w1psficore = w1psficore + f1ac*w1psfca + f1ap*w1psfpa
c
      hdrline(1)  = 'EXTEND  =                    T /'
...
c
     ...
     +          Ang1dpMin,w1psfpd,w1psfuncpd,wgt)
      w1psficore = w1psficore + f1dc*w1psfcd + f1dp*w1psfpd
c         
      OutFNam = OutPath(1:LNBlnk(OutPath))
...
c
     ...
     +          Ang2apMin,w2psfpa,w2psfuncpa,wgt)
      w2psficore = w2psficore + f2ac*w2psfca + f2ap*w2psfpa
c      
      OutFNam = OutPath(1:LNBlnk(OutPath))
...
c
     ...
     +          Ang2dpMin,w2psfpd,w2psfuncpd,wgt)
      w2psficore = w2psficore + f2dc*w2psfcd + f2dp*w2psfpd
c      
      OutFNam = OutPath(1:LNBlnk(OutPath))
...
c     FPALOCY =                  508    
c
      call DownSamp(w1psficore,nOvrSamp,w1psfawaic)
      call DownSamp(w2psficore,nOvrSamp,w2psfawaic)
c     
      hdrline(1)  = 'EXTEND  =                    T /'
     +              //' TAPE MAY HAVE STANDARD FITS EXTENSIONS'         
      hdrline(2)  = 'CDELT1  =       -0.00019097222 /'
      hdrline(3)  = 'CDELT2  =        0.00019097222 /'
      hdrline(4)  = 'CRPIX1  =               14.000 /'
      hdrline(5)  = 'CRPIX2  =               14.000 /'
      hdrline(6)  = 'FPALOCX =                  508 /'
      hdrline(7)  = 'FPALOCY =                  508 /'
      write(hdrline(8), '(''NOVRSAMP=           '',I10,'' /'')')
     +      nOvrSamp
      hdrline(9)  = 'TILE    = '''//TileNam//''''
      naxes(1) = 27
      naxes(2) = 27
      NPix     = 729
      OutFNam  = OutPath(1:LNBlnk(OutPath))
     +           //'unwise-w1-psf-awaic-01x01-01x01.fits'
      call wrfitsr(2,naxes,NPix,w1psfawaic,OutFNam,status,9,hdrline)
      if (status .ne.0) then
        print *,'ERROR: output status = ',status,' on file:'
        print *,OutFNAM(1:lnblnk(OutFNam))
      else        
     ...
     +           //'unwise-w2-psf-awaic-01x01-01x01.fits'
      call wrfitsr(2,naxes,NPix,w2psfawaic,OutFNam,status,8,hdrline)
      if (status .ne.0) then
        print *,'ERROR: output status = ',status,' on file:'
        print *,OutFNAM(1:lnblnk(OutFNam))
      else        
...
c-----------------------------------------------------------------------
c
      integer       status,blocksize,outunit,naxis,naxes(naxis),bitpix,
     +              fpixel, group, nelements, nhdrline, n 
      real(4)       array(lsize)
      character*(*) fout
